...
import random
igdm = ("This is your first time running the script , run again to get unfollowers/added.")
...
def scrape(username):
    ...
    if os.path.exists('instaTrackDM.txt'):
        greenPrint("\nPrinting in Instagram DM shortly!\n")
        ...
        bot.find_element_by_xpath(
        '/html/body/div[1]/section/main/div/header/section/div[2]/div/div/div[1]/div/button').click() 
        time.sleep(2) 
        bot.find_element_by_xpath(
        ...
        '/html/body/div[1]/section/div[2]/div/div/div[2]/div/div/div[2]/button').click() 
        time.sleep(1.2) 
        igcreate = open("instaTrackDM.txt","a+")
        igcreate.close()
        lastClose()
scrape(usr)
...
def scrape(username):
    ...
    if os.path.exists('instaDiscord.txt'):
        greenPrint("\nPrinting in discord shortly!\n")
        instaDiscord = open("instaDiscord.txt", "w+")
        ...
        for count, addedusr in enumerate(added):
            instaDiscord.write(("{:02d}: {}".format(count+1, addedusr)))
        ...
        hook.send(disc)     
        dcreate = open("instaDiscord.txt","a+")
        dcreate.close()
        lastClose()
scrape(usr)
